...
WebserverManager::WebserverManager(WifiManager *wifiManager, std::function<String()> getCurrentStatus, std::function<String()> getVersion)
{
...
void WebserverManager::_setupWebServer()
{
    ...
    _server.on("/version", HTTP_GET, [this]()
                {
                    if (this->_getVersion == nullptr) {
                        _server.send(200, "text/plain", "No version available");
                        return;
                    }
                    _server.send(200, "text/plain", this->_getVersion());
                });
